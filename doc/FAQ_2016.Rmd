...
```{r}
## Keep the code indented as-is
opts_chunk$set(tidy = FALSE)

## Do not evaluate examples by default, but do echo text
## Don't warn and don't print messages
...
##------------------------------------------------------------------------------
## KNITR SPECIFIC INITIALIZATIONS
##------------------------------------------------------------------------------


## Set directory to root dir before loading files
geneorama::set_project_dir("geneorama")

## Load project functions
sourceDir("code/functions/")
some_data <- readRDS("data/some_data.Rds")
...
# all the point types
nn <- c(1:25, 32:50);    plot(nn,nn,type='n');for(i in nn)points(i,i,pch=i);rm(i,nn)
...
op <- par(mfrow=c(2,2), mar=c(0,0,1.5,0))
...
filled.contour(x = MASS::kde2d(a,b,n=100), 
...
## Cookbook - Load data from Yahoo
```{r, eval=TRUE, results="hold"}
geneorama::loadinstall_libraries(c("fImport", "data.table", "geneorama"))
...
## Cookbook - Check R scripts for presence of a word pattern
...
## Cookbook for .First function (use with caution)
...
## Cookbook transfer libraries from a new R installation
